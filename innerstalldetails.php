...
<div class="theme-layout">
						...
						<div class="listingcomment">
							<div class="leavecomment">
								...
								<div class="upimg">
										...
										<input type="file" name="revimage" />
									</div>
								
								
									<input name="title" placeholder="Title" type="text">
									<textarea name="description" placeholder="Review"></textarea>
									<input name="submit" value="Submit Review" type="submit">
								</form>
							</div>
						</div>
					</div>
	...
	<?php
		if(isset($_POST['submit']))
		{
			...
			$query7 = "SELECT uid FROM user_register WHERE uemail = '$_SESSION[uemail]' ";
			...
			$path = "C:\\xampp\\htdocs\\Street\\review_images\\";

			$target_file_0 = $path.basename($_FILES["revimage"]["name"]);

    		$review_image = $_FILES['revimage']['name'];
    
    		move_uploaded_file($_FILES['revimage']['tmp_name'],"C:\\xampp\\htdocs\\Street\\review_images\\".$_FILES['revimage']['name']);

			$query8 = "INSERT INTO review(user_id,stall_id,title,description,image,date) VALUES('$uid','$stall_id','$title','$description','$review_image',NOW())";
			$result8 = mysqli_query($conn,$query8);
			if($result8)
			{

				echo"  <script> swal('Success', 'Review Given Successfull! ', 'success'); </script>";
			}
			else
			{
				echo"  <script> swal('Sorry!', 'Review Not Added! ', 'error'); </script>";
			}
